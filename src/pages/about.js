...
const About = () => {
    return (
        <div className='about' id='main'>
            ...
            <h4>
                Design choice: Editing alt text
            </h4>
            <p>
                Because AI can be inaccurate, we created an option for users to self-submit alt text in case neither of the alt text generated by the AI are correct or relevant. 
            </p>
            <h3>
                Developing alti
            </h3>
            ...
            <p>
                If we had more time, we would run a study with BLV people to learn more about their thoughts on using Alti. That way, we can better cater to the bot to their needs and get feedback about their thoughts on using AI to generate alt text.
            </p>
            <p>
            ...
            <ul>
                ...
                <li>Alti currently works with GIFs in limited scenarios, and only takes the first frame of the GIF to generate alt text. It would be ideal if Alti could generate alt text for the entire GIF rather than the first frame.</li>
            </ul>
            <h2>
                How we made Alti accessible
            </h2>
            <p>
                Alti can be added to a Discord server by clicking the "Add to Discord" button in the navigation bar. We have checked this site with both automated tools such as the Web Accessibility Evaluation Tool (WAVE) and manual testing with screen readers. We added alt text to all images when necessary. Because this site features so many images with text descriptions displayed on the site itself, we made sure to use blank alt text or use ARIA hidden labels for those images so that screen reader users wouldn't have to hear the same image description twice. We also made sure to use semantic HTML and to have a high contrast between text and background colors. Also, we added a skip link for users who use tab navigation. Our heading levels have all been manually checked to make sure that they are in the correct order. 
            </p>
            <p>
