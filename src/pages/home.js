...
const HomePage = () => {
  ...
  const bunClick = () => {
    buntext === '?' ? setBuntext('A light brown, toasted hamburger bun.') : setBuntext('?');
  }

  const meatClick = () => {
    meattext === '?' ? setMeattext('A hamburger patty with grill marks.') : setMeattext('?');
  }

  const vegClick = () => {
    vegtext === '?' ? setVegtext('A set of green lettuce leaves, a red tomato slice, and purple sliced onions.') : setVegtext('?');
  }
        ...
        <Row style={{ marginRight: 0, marginLeft: 0 }}>
          <Col sm={8}>
            <Container className='p-4 example-img-container'>
              <Image src={'img/burger.jpg'} alt="burger" className="layer-parent" fluid />
              <Button type='checkbox' className="layer-bun circular-btn" onClick={bunClick}>{buntext}</Button>
              <Button type='checkbox' className="layer-meat circular-btn" onClick={meatClick}>{meattext}</Button>
              <Button type='checkbox' className="layer-vegetables circular-btn" onClick={vegClick}>{vegtext}</Button>
              <Button type='checkbox' className="layer-bag circular-btn" onClick={bagClick}>{bagtext}</Button>
              <p className="img-caption pt-4">Click on the question marks for examples of alt text that describe that particular part of the image.</p>
            </Container>
          </Col>
          <Col sm={4}>
